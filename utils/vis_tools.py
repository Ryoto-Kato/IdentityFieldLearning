...
def VisTrimesh(tri_meshes, colors=None, off_screen = True, camera_extrinsic = None, style = 'points'):
    """
    input: args
        - path2mesh
        - off_screen
    ...
    for tri_mesh, color in zip(tri_meshes, colors):
        ...
        mesh = pv.wrap(tri_mesh)
        if color != None:
            p.add_mesh(mesh, color = color, style=style, opacity = 1.0)
        else:
            p.add_mesh(mesh, color = trimesh.visual.random_color(), style = style, opacity = 1.0)
