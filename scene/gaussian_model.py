...
from utils.ply_helper import tex2vertsAttributes
...
@dataclass
class torch_GaussianProp:
        """
        f_dc is SH_coeffs not RGB!!
        """
        xyz:torch.Tensor
        f_dc:torch.Tensor #this is SH_coeffs, needs to be converted to RGB by SH2RGB
        f_rest:torch.Tensor
...
class GaussianModel:
    ...
    def get_SH(self):
        """
        where _feature_dc [render_numGauss, int(sh_deg+1)**2//3, 3]
        """
        return self._features_dc #[render_numGass, int(sh_deg+1)**2//3, 3]
    ...
    def GaussianProp_from_pcd(self, pcd : BasicPointCloud):
        ...
        return self.conver2torch_GaussianProp()
    
    def create_from_learnable_blenshape(self, xyz, fdc, frest, scale, rot, opac):
        self._xyz = xyz
        # print("xyz", self._xyz.shape, self._xyz.requires_grad)
        self._features_dc = fdc.contiguous() #[render_numGauss, 3] for RGB, [3007, int(sh_deg+1)**2//3, 3] for SH
        # print("fdc", self._features_dc.shape, self._features_dc.requires_grad)

        self._features_rest = frest
        # print("frest", self._features_rest.shape, self._features_rest.requires_grad)

        self._scaling = scale
        # print("scale", self._scaling.shape, self._scaling.requires_grad)
        self._rotation = rot
        # print("rot", self._rotation.shape, self._rotation.requires_grad)
        self._opacity = opac
        # print("opac", self._opacity.shape, self._opacity.requires_grad)
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
    
    def create_from_uv(self, UVs, uv_map, index_in_batch, learnable_uv = True, ref_tracking_points = None):
        """
        #Attribute: 59(23, 38) (xyz:3, f_dc:3, f_rest:45(9:sh degree 1, 24:sh degree 2), opacity:1, scale:3, rotation:4)
        # UVs: [B, 23, 512, 512]
        # uvmap: should be [0, 1]
        # B: #number of subjects/batches
        """

        if learnable_uv:
            num_Gaussians = uv_map.shape[1]
            if ref_tracking_points != None:
                # fused_point_cloud = uv_map[index_in_batch, :, :3]
                # scales = uv_map[index_in_batch, :, 16:19]
                # rots = uv_map[index_in_batch, :, 19:23]
                
                fused_point_cloud=ref_tracking_points[:num_Gaussians].cuda()
                dist2 = torch.clamp_min(distCUDA2((ref_tracking_points[:num_Gaussians]).cuda()), 0.0000001)
                scales = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)
                rots = torch.zeros((fused_point_cloud.shape[0], 4), device="cuda")
                rots[:, 0] = 1
                # scales = uv_map[index_in_batch, :, 16:19]
                # rots = uv_map[index_in_batch, :, 19:23]
                # fused_point_cloud = uv_map[index_in_batch, :, :3]
                # ref_tracking_points = ref_tracking_points.cpu()
            else:
                fused_point_cloud = uv_map[index_in_batch, :, :3]
                scales = uv_map[index_in_batch, :, 16:19]
                rots = uv_map[index_in_batch, :, 19:23]

            fused_color = RGB2SH(uv_map[index_in_batch, :, 3:6])
            fused_rest = uv_map[index_in_batch, :, 6:15]
            self.max_sh_degree = 1
            features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).cuda()
            features[:, :, 0] = fused_color
            features[:, :, 1:] = fused_rest.reshape(num_Gaussians, 3, -1)
            opacities = uv_map[index_in_batch, :, 15:16]
        else:
            xyz_UV = UVs[index_in_batch, :3, :, :]
            f_dc_UV = UVs[index_in_batch, 3:6, :, :]
            f_rest_UV = UVs[index_in_batch, 6:15, :, :]
            opacity_UV = UVs[index_in_batch, 15:16, :, :]
            scale_UV = UVs[index_in_batch, 16:19, :, :]
            rot_UV = UVs[index_in_batch, 19:23, :, :]

            B, num_channels, max_u, max_v = UVs.shape

            # print(f"xyz UV: {xyz_UV.shape}")
            # print(f"f_dc_UV: {f_dc_UV.shape}")
            # print(f"f_rest_UV: {f_rest_UV.shape}")
            # print(f"opacity_UV: {opacity_UV.shape}")
            # print(f"scale_UV: {scale_UV.shape}")
            # print(f"rot_UV: {rot_UV.shape}")

            # print("initialize from point clouds (.ply) in directory")
            
            fused_point_cloud = tex2vertsAttributes(uv_map=uv_map, UV=xyz_UV, max_u=max_u, max_v=max_v, index_in_batch=index_in_batch).cuda()
            num_Gaussians = fused_point_cloud.shape[0]
            # print(f"fused_point: {fused_point_cloud.shape}")
            fused_color = RGB2SH(tex2vertsAttributes(uv_map=uv_map, UV=f_dc_UV, max_u=max_u, max_v=max_v, index_in_batch=index_in_batch)).cuda()
            # print(f"fused_color: {fused_color.shape}")

            fused_rest = tex2vertsAttributes(uv_map=uv_map, UV=f_rest_UV, max_u=max_u, max_v=max_v, index_in_batch=index_in_batch).cuda()
            fused_rest = fused_rest.reshape(num_Gaussians, 3, -1)
            # print(f"fused_rest: {fused_rest.shape}")
            # print(f"max_sh_degree: {self.max_sh_degree}")
            self.max_sh_degree = 1
            features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).cuda()
            features[:, :, 0] = fused_color
            features[:, :, 1:] = fused_rest

            # print("Number of points at initialisation : ", fused_point_cloud.shape[0])

            scales = tex2vertsAttributes(uv_map=uv_map, UV=scale_UV, max_u=max_u, max_v=max_v, index_in_batch=index_in_batch).cuda()
            # print(f"scales: {scales.shape}")
            rots = tex2vertsAttributes(uv_map=uv_map, UV=rot_UV, max_u=max_u, max_v=max_v, index_in_batch=index_in_batch).cuda()
            # print(f"rots: {rots.shape}")
            opacities = tex2vertsAttributes(uv_map=uv_map, UV=opacity_UV, max_u=max_u, max_v=max_v, index_in_batch=index_in_batch).cuda()
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        # self._xyz = nn.Parameter(data = fused_point_cloud, requires_grad = False)
        self._features_dc = nn.Parameter(features[:,:,0:1].transpose(1, 2).contiguous().requires_grad_(True))
        print(f"f_dc: {self._features_dc.shape}")
        
        self._features_rest = nn.Parameter(features[:,:,1:].transpose(1, 2).contiguous().requires_grad_(True))
        print(f"f_rest: {self._features_rest.shape}")
        
        self._scaling = nn.Parameter(scales.requires_grad_(True))
        print(f"scale: {self._scaling.shape}")
    
        self._rotation = nn.Parameter(rots.requires_grad_(True))
        print(f"rot: {self._rotation.shape}")

        self._normal = nn.Parameter(normals.requires_grad_(True))

        # TODO: Do not optimize opacity (we assume opacity=1.0)
        self._opacity = nn.Parameter(data = opacities.requires_grad_(False), requires_grad=True)
        print(f"opacity: {self._opacity.shape}")
    ...
    def save_GaussianProp(self, path):
        ...
        if self.max_sh_degree > 0:
            if self._features_rest != None:
                f_rest = self._features_rest.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
                print("f_rest",f_rest.shape)
        else:
            f_rest = np.empty(0) #np.zeros((f_dc.shape[0], int((((self.max_sh_degree+1)**2)*3-3)/3), 3)).float().cuda()
    ...
    def conver2torch_GaussianProp(self):
        xyz = self._xyz
        ...
        gs_prop = torch_GaussianProp(xyz=xyz, f_dc=f_dc, f_rest=f_rest, opacities=opacities, scale=scale, rotation=rotation)
        return gs_prop
    ...
    def save_ply(self, path):
        ...
        if self.max_sh_degree > 0:
            ...
            if self._features_rest == None:
                f_rest = torch.zeros(self.render_numGauss, int((((self.max_sh_degree+1)**2)*3-3)/3), 3).float().cuda()
